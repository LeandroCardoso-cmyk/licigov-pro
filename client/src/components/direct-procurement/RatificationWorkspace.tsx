  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); ratify.mutate({ workspaceId, decision, justification: justification || undefined, evidence: evidence ? evidence.split("\n").filter(Boolean) : undefined }); }}
      ...
      <div className="inline-flex rounded-lg bg-muted p-0.5 text-xs font-medium">
        <button type="button" onClick={() => setDecision("ratificado")} className={`rounded-md px-3 py-1 transition ${decision === "ratificado" ? "bg-card text-green-700 shadow-sm" : "text-muted-foreground"}`}>Ratificar</button>
        <button type="button" onClick={() => setDecision("nao_ratificado")} className={`rounded-md px-3 py-1 transition ${decision === "nao_ratificado" ? "bg-card text-red-700 shadow-sm" : "text-muted-foreground"}`}>Não ratificar</button>
      </div>
