...
export default function RatificationWorkspace({ workspaceId, onRatified }: RatificationWorkspaceProps) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); ratify.mutate({ workspaceId, decision, justification: justification || undefined, evidence: evidence ? evidence.split("\n").filter(Boolean) : undefined }); }}
      ...
      <h3 className="text-sm font-semibold text-gray-900">Ratificação</h3>
      <div className="inline-flex rounded-lg bg-gray-100 p-0.5 text-xs font-medium">
        <button type="button" onClick={() => setDecision("ratificado")} className={`rounded-md px-3 py-1 transition ${decision === "ratificado" ? "bg-white text-green-700 shadow-sm" : "text-gray-500"}`}>Ratificar</button>
        <button type="button" onClick={() => setDecision("nao_ratificado")} className={`rounded-md px-3 py-1 transition ${decision === "nao_ratificado" ? "bg-white text-red-700 shadow-sm" : "text-gray-500"}`}>Não ratificar</button>
      </div>
      ...
      <textarea value={evidence} onChange={(e) => setEvidence(e.target.value)} rows={2} placeholder="Evidências (uma por linha)…"
        className="w-full resize-y rounded-md border border-gray-300 px-2 py-1.5 text-sm focus:border-indigo-400 focus:outline-none" />
      {ratify.isSuccess && <p className="text-xs text-green-700">Ratificação registrada.</p>}
