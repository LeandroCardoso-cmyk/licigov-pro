  ...
  return (
    <section className="rounded-xl border border-gray-200 bg-gray-50 p-4">
      <header className="mb-4">
        <h2 className="text-base font-semibold text-gray-900">Caixa Institucional — Parecer Jurídico</h2>
        <p className="text-xs text-gray-500">Solicitações encaminhadas pelo Institutional Request Engine. Documentos referenciados, nunca copiados.</p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="rounded-lg bg-white/60 p-3">
          ...
          <div className="space-y-3">
            {isLoading ? (
              ...
              <p className="rounded-md border border-dashed border-gray-200 p-4 text-center text-xs text-gray-400">Nenhuma solicitação pendente.</p>
            ) : (
              requests.map((r) => (
        ...
        <div className="rounded-lg bg-white/60 p-3">
          <h3 className="mb-3 flex items-center justify-between text-sm font-semibold text-gray-700">
            Em andamento
            <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-normal text-gray-500">{isLoading ? "…" : workspaces.length}</span>
          </h3>
          <div className="space-y-3">
            {workspaces.length === 0 ? (
              <p className="rounded-md border border-dashed border-gray-200 p-4 text-center text-xs text-gray-400">Nenhum trabalho aberto.</p>
            ) : (
              workspaces.map((w) => (
                <button key={w.id} type="button" onClick={() => onOpenWorkspace?.(w.id)}
                  className="w-full rounded-lg border border-gray-200 bg-white p-3 text-left transition hover:border-indigo-300">
                  <div className="mb-1 flex items-center justify-between">
                    <span className="text-xs text-gray-500">{domainLabel(w.sourceDomain)}</span>
                  ...
                  <p className="text-xs text-gray-400">Atualizado em {formatDate(w.updatedAt)}</p>
                </button>
              ))
            )}
          </div>
