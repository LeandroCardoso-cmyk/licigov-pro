  ...
  return (
    <div className="rounded-lg border border-indigo-100 bg-indigo-50/40 p-4">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-indigo-900">Copilotos (supervisionados)</h3>
        {recommendation && (
      ...
      {!recommendation ? (
        <p className="text-xs text-gray-500">{busy ? "Consultando copilotos…" : "Nenhuma recomendação gerada ainda. Os copilotos nunca decidem — apenas sugerem."}</p>
      ) : (
        <div className="space-y-2 text-xs">
          <p className="text-gray-700"><strong>Reasoning:</strong> {recommendation.reasoning}</p>
          <p className="text-gray-600"><strong>Explainability:</strong> {recommendation.explainability}</p>
          <p className="text-gray-500"><strong>Provenance:</strong> {recommendation.provenance}</p>
          <div className="flex gap-2 pt-1">
            ...
            <button type="button" onClick={onReject} className="rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50">Rejeitar</button>
          </div>
          <p className="pt-1 text-[11px] italic text-indigo-700">Sugestão revisável — nunca automática.</p>
        </div>
      )}
    </div>
  );
