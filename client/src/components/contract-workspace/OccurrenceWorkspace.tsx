...
export default function OccurrenceWorkspace({ contractId, occurrences = [] }: OccurrenceWorkspaceProps) {
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border bg-card p-4">
      <h3 className="text-sm font-semibold text-foreground">Ocorrências</h3>
        ...
        <ul className="divide-y divide-border">
          {occurrences.map((o) => (
            <li key={o.id} className="flex items-start justify-between gap-2 py-2 text-sm">
              <div><p className="text-foreground">{o.description}</p>{o.notes && <p className="text-xs text-muted-foreground">{o.notes}</p>}</div>
              <span className="shrink-0 text-[11px] text-muted-foreground">{formatDate(o.createdAt)}</span>
            </li>
          ))}
        </ul>
      )}
