...
Os 59 achados agrupados em **4 blocos**. Ordem recomendada: **A → B → D** antes do piloto;
...
`.env.example`; considerar backup agendado.
...
responde HTTP 200; retry não duplica; IA nunca serve mock silenciosamente como oficial.
...
**Estimativa total:** ~4 PRs. **Caminho mínimo até o piloto:** Blocos A + B + D.
