...
| G5 | Segredos fora do repositório e rotacionados | **PASS** | Rotação operacional do `JWT_SECRET` de produção **executada pelo operador** e **validada** em 2026-09-21 (ver [`G5_SECRET_READINESS_AUDIT.md`](../../security/G5_SECRET_READINESS_AUDIT.md), adendo "Atestação"). Novo deployment `120edffb` **SUCCESS** (healthcheck `/readyz` PASS ⇒ readyz=ok + database=ok; boot sem erro/fallback de `JWT_SECRET` — fail-closed); login novo confirmado (sessões antigas invalidadas); reference installer **noop/replay-safe** (hash `332a9cb3…`, sem ativação). Código já era `CODE_COMPLETE` (só `.env.example`, fail-closed); `GEMINI_API_KEY` já rotacionada (G12); `DATABASE_URL` histórica sem credencial inline. Evidência **sem valor de segredo** |
...
| G8 | Fluxo principal navegável e sem telas de debug/duplicadas | **PASS** | Consolidação canônica governada (2026-09-20, ver [`G8_PILOT_NAVIGATION_AUDIT.md`](./G8_PILOT_NAVIGATION_AUDIT.md), adendo "Fechamento"). O canônico **absorve a criação/detalhe contextual** (rotas `/parecer/novo`, `/contratos/novo`, `/contratacao-direta/novo` + `:id`/`analytics`), **reusando** os componentes/routers existentes (sem arquitetura paralela). Núcleo migrado: `ProcessDetails`/`DirectContractDetails` criam parecer/contrato por **rota canônica preservando `processId`/`contractId`**. Rotas legadas (`/parecer-juridico`, `/contracts`, `/direct-contracts`) → **redirect de compatibilidade** preservando `:id`/query (sem loop); detalhes leem `:id` via `useParams` (agnóstico à rota); auth/tenant preservados; nav sem legados; `/test*` fora. Guarda: `g8-canonical-navigation.test.ts`. Ressalva operacional (não bloqueante): recomendável smoke visual dos fluxos em produção pós-deploy |
...
>
> **Atualização (2026-09-20 — fechamento canônico do G8):** **G8 → `PASS`** (consolidação canônica
> governada: criação/detalhe contextual absorvidos pelo canônico reusando componentes existentes;
> legados viram redirect de compatibilidade; núcleo migrado; guarda `g8-canonical-navigation.test.ts`).
> Estado corrente: **11 PASS · 0 FAIL · 1 PARTIAL** — único bloqueante restante: **G5** (rotação
> operacional do `JWT_SECRET` no Railway — ação de operador, não executável pelo repositório).
>
> **Atualização (2026-09-21 — fechamento do G5):** **G5 → `PASS`** — rotação do `JWT_SECRET` de
> produção executada pelo operador e validada (deployment `120edffb` SUCCESS; `/readyz` PASS; DB ok;
> login novo confirmado; boot fail-closed sem erro; reference installer noop/replay-safe; sem valor de
> segredo exposto). **Estado FINAL do Gate Obrigatório: 12 PASS · 0 FAIL · 0 PARTIAL (12/12).**
> Classificação técnica: **PILOT TECHNICALLY READY FOR OWNER DECISION** — a IA não autoriza o piloto;
> a decisão de go-live é do owner.
