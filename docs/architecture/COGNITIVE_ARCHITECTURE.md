...
## Fundação normativa da Lei nº 14.133 (RC-4.6.1)

Sobre o pacote federal, a **Normative Foundation** (`server/domain/legal/normative/`) inicia oficialmente o Corpus Federal da Lei nº 14.133/2021: **hierarquia normativa oficial** (Lei → Livro → Título → Capítulo → Seção → Subseção → Artigo → Parágrafo → Inciso → Alínea → Item), **NormativeNode** (árvore estrutural reutilizável para qualquer norma), **cross references** (referência/remissão/dependência/correlação/regulamentadora), projeção KG, consultas declarativas e explainability. A ligação `NormativeNode → LegalKnowledgeUnit` está preparada mas **null** nesta fase (conteúdo virá incrementalmente). **NÃO insere o texto da Lei 14.133.** Ver [FEDERAL_PROCUREMENT_CORPUS_FOUNDATION.md](./FEDERAL_PROCUREMENT_CORPUS_FOUNDATION.md).
...
## Inicialização da plataforma (RC-X.2)

O **Institutional Bootstrap Framework** (`server/domain/bootstrap/`) orquestra a inicialização dos três pilares (Cognitivo, Experiência e Business Domains): **Bootstrap Kernel**, **Pipeline** declarativo (Authentication → Institution Context → Corpus → Package → Capability → Workspace → Navigation → Home → Copilot → Business → Ready), **Dependency Graph** (ordem determinística, sem ciclos), **Platform State**, **Registry**, **Health** e **Context Reload**. Cada novo módulo registra apenas Stage/Dependencies/Initializer/HealthCheck. Multi-tenant, replay-safe, explicável, observável. **NÃO executa regra de negócio, NÃO contém IA, NÃO conhece Lei 14.133.** Ver [INSTITUTIONAL_BOOTSTRAP_FRAMEWORK.md](./INSTITUTIONAL_BOOTSTRAP_FRAMEWORK.md).
