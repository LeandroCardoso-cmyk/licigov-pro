...
async function seed() {
  ...
  try {
    // 1. Criar usuários de teste (órgãos públicos)
    ...
    // 2. Buscar IDs dos usuários e planos
    const allUsers = await db.select().from(users);
    const allPlans = await db.select().from(subscriptionPlans);

    if (allUsers.length === 0 || allPlans.length === 0) {
      throw new Error("Usuários ou planos não encontrados");
    }
    ...
    // 3. Criar assinaturas via empenho com diferentes status
    ...
    for (const sub of subscriptionsData) {
      ...
      createdSubscriptions.push({ ...sub, id: Number(result.insertId) });
    }
    console.log("✅ Assinaturas criadas\n");

    // 4. Criar parcelas mensais para cada assinatura
    ...
    // 5. Criar histórico de renovações para contratos com renewalCount > 0
