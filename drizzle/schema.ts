...
// Consolida (por referência, sem duplicar) os Business Domains. Só possui dados
// próprios para o que NÃO existe nos domínios: registros legados/manuais, eventos
// de calendário, marcos externos, timeline operacional, publicações e config.

export const operationRecordsTable = mysqlTable("operation_records", {
  id:              varchar("id", { length: 20 }).notNull().primaryKey(),
  organizationId:  int("organization_id").notNull(),
  recordType:      varchar("record_type", { length: 40 }).notNull().default("outro"),
  origin:          varchar("origin", { length: 10 }).notNull().default("externa"),
  number:          varchar("number", { length: 80 }).notNull().default(""),
  object:          text("object"),
  modality:        varchar("modality", { length: 60 }).notNull().default(""),
  currentStage:    varchar("current_stage", { length: 60 }).notNull().default(""),
  responsible:     int("responsible"),
  referenceType:   varchar("reference_type", { length: 40 }).notNull().default(""),
  referenceId:     varchar("reference_id", { length: 64 }).notNull().default(""),
  documentReferences: text("document_references"),
  notes:           text("notes"),
  correlationId:   varchar("correlation_id", { length: 64 }).notNull().default(""),
  createdAt:       datetime("created_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
...
export const operationalEventsTable = mysqlTable("operational_events", {
  id:              varchar("id", { length: 20 }).notNull().primaryKey(),
  organizationId:  int("organization_id").notNull(),
  eventType:       varchar("event_type", { length: 40 }).notNull().default("manual"),
  title:           varchar("title", { length: 500 }).notNull().default(""),
  eventDate:       varchar("event_date", { length: 10 }).notNull().default(""),
  eventTime:       varchar("event_time", { length: 5 }).notNull().default(""),
  referenceType:   varchar("reference_type", { length: 40 }).notNull().default(""),
  referenceId:     varchar("reference_id", { length: 64 }).notNull().default(""),
  autoGenerated:   int("auto_generated").notNull().default(0),
  alertOffsetDays: int("alert_offset_days").notNull().default(0),
  correlationId:   varchar("correlation_id", { length: 64 }).notNull().default(""),
  createdAt:       datetime("created_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
});

export const operationalMilestonesTable = mysqlTable("operational_milestones", {
  id:              varchar("id", { length: 20 }).notNull().primaryKey(),
  organizationId:  int("organization_id").notNull(),
  referenceType:   varchar("reference_type", { length: 40 }).notNull().default(""),
  referenceId:     varchar("reference_id", { length: 64 }).notNull().default(""),
  milestoneType:   varchar("milestone_type", { length: 30 }).notNull().default("outro"),
  date:            varchar("date", { length: 10 }).notNull().default(""),
  time:            varchar("time", { length: 5 }).notNull().default(""),
  result:          varchar("result", { length: 255 }).notNull().default(""),
  observation:     text("observation"),
  correlationId:   varchar("correlation_id", { length: 64 }).notNull().default(""),
  createdAt:       datetime("created_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
});

export const operationalTimelineTable = mysqlTable("operational_timeline", {
  id:              varchar("id", { length: 20 }).notNull().primaryKey(),
  organizationId:  int("organization_id").notNull(),
  eventOrder:      int("event_order").notNull().default(0),
  actor:           varchar("actor", { length: 60 }).notNull().default(""),
  action:          varchar("action", { length: 60 }).notNull().default(""),
  referenceType:   varchar("reference_type", { length: 40 }).notNull().default(""),
  referenceId:     varchar("reference_id", { length: 64 }).notNull().default(""),
  summary:         text("summary"),
  correlationId:   varchar("correlation_id", { length: 64 }).notNull().default(""),
...
export const publicationRecordsTable = mysqlTable("publication_records", {
  id:              varchar("id", { length: 20 }).notNull().primaryKey(),
  organizationId:  int("organization_id").notNull(),
  referenceType:   varchar("reference_type", { length: 40 }).notNull().default(""),
  referenceId:     varchar("reference_id", { length: 64 }).notNull().default(""),
  channel:         varchar("channel", { length: 30 }).notNull().default("pncp"),
  status:          varchar("status", { length: 20 }).notNull().default("nao_iniciado"),
  date:            varchar("date", { length: 10 }).notNull().default(""),
  correlationId:   varchar("correlation_id", { length: 64 }).notNull().default(""),
  createdAt:       datetime("created_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
  updatedAt:       datetime("updated_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
...
export const operationalSettingsTable = mysqlTable("operational_settings", {
  id:                 varchar("id", { length: 20 }).notNull().primaryKey(),
  organizationId:     int("organization_id").notNull(),
  orgaoOficialName:   varchar("orgao_oficial_name", { length: 255 }).notNull().default("Órgão Oficial do Município"),
  jornalName:         varchar("jornal_name", { length: 255 }).notNull().default("Jornal de Grande Circulação"),
  portalName:         varchar("portal_name", { length: 255 }).notNull().default("Portal Eletrônico"),
  correlationId:      varchar("correlation_id", { length: 64 }).notNull().default(""),
  createdAt:          datetime("created_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
  updatedAt:          datetime("updated_at", { mode: "string", fsp: 3 }).default(sql`CURRENT_TIMESTAMP(3)`).notNull(),
});
