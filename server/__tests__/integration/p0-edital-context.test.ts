...
const base = (over: Partial<EditalSourceInputs> = {}): EditalSourceInputs => ({
  ...
  approvedItems: [
    { id: "i1", description: "Papel A4", quantity: 100, unit: "resma", averagePrice: 2550, suggestedCATMAT: "12345" },
  ],
...
describe("P0 — buildEditalSourceContext (reuso estruturado)", () => {
  ...
  it("digest é determinístico e SENSÍVEL a mudança de fonte (base para replay + SOURCE_CHANGED)", () => {
    ...
    expect(buildEditalSourceContext(base({ approvedItems: [{ id: "i1", description: "Papel A4 90g", quantity: 100, unit: "resma", averagePrice: 2550, suggestedCATMAT: "12345" }] })).sourcesDigest).not.toBe(a);
  });

  it("digest independe da ORDEM dos itens (assinatura ordenada internamente)", () => {
    const items = [
      { id: "i1", description: "Papel A4", quantity: 100, unit: "resma", averagePrice: 2550, suggestedCATMAT: "12345" },
      { id: "i2", description: "Caneta", quantity: 50, unit: "un", averagePrice: 120, suggestedCATMAT: null },
    ];
