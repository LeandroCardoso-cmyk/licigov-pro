...
export const BOUNDARY_CLASSIFICATIONS: readonly BoundaryClassificationEntry[] = [
  ...
  // A3 — server/services/directContractDocuments.ts MIGRADO: agora chama executeCognitiveTask
  ...
  { path: "server/routers/legalOpinionsRouter.ts", allowlist: "LEGACY_ACTIVE_MAINTENANCE_ONLY", disposition: "manutencao_apenas", note: "13/15 procedures em tenantProcedure (2 deliberadamente protectedProcedure — escopo por usuário). generateOpinion/create validam as 4 origens do enum sourceType (contract/process/direct_contract/other) via getContractByIdForOrganization/getProcessByIdForOrganization/getDirectContractByIdForOrganization. Ver LEGACY_INVENTORY.md § LegalOpinions." },
